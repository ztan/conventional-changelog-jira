...
betterThanBefore.setups([
  function () {
    ...
    gitDummyCommit(['DIGI-1234: build: first build setup', 'BREAKING CHANGE: New build system.'])
    gitDummyCommit(['[DIGI-1234]: ci(travis): add TravisCI pipeline', 'BREAKING CHANGE: Continuously integrated.'])
    gitDummyCommit(['feat: amazing new module', 'BREAKING CHANGE: Not backward compatible.'])
    gitDummyCommit(['[DIGI-1234] fix(compile): avoid a bug', 'BREAKING CHANGE: The Change is huge.'])
    gitDummyCommit(['DIGI-1234 perf(ngOptions): make it faster', ' closes #1, #2'])
  ...
  function () {
    gitDummyCommit(['DIGI-1234: feat(awesome): issue brought up by @bcoe! on Friday'])
  },
  function () {
    gitDummyCommit(['DIGI-1234: build(npm): edit build script', 'BREAKING CHANGE: The Change is huge.'])
    gitDummyCommit(['DIGI-1234: ci(travis): setup travis', 'BREAKING CHANGE: The Change is huge.'])
    gitDummyCommit(['DIGI-1234: docs(readme): make it clear', 'BREAKING CHANGE: The Change is huge.'])
    gitDummyCommit(['DIGI-1234: style(whitespace): make it easier to read', 'BREAKING CHANGE: The Change is huge.'])
    gitDummyCommit(['DIGI-1234: refactor(code): change a lot of code', 'BREAKING CHANGE: The Change is huge.'])
    gitDummyCommit(['DIGI-1234: test(*): more tests', 'BREAKING CHANGE: The Change is huge.'])
  },
  function () {
  ...
  function () {
    gitDummyCommit(['DIGI-1234: feat(*): implementing #5 by @dlmr', ' closes #10'])
  },
  function () {
    gitDummyCommit(['DIGI-1234: fix: use npm@5 (@username)'])
    gitDummyCommit(['DIGI-1234: build(deps): bump @dummy/package from 7.1.2 to 8.0.0', 'BREAKING CHANGE: The Change is huge.'])
  },
  function () {
    gitDummyCommit(['DIGI-1234: Revert \\"feat: default revert format\\"', 'This reverts commit 1234.'])
    gitDummyCommit(['DIGI-1234: revert: feat: custom revert format', 'This reverts commit 5678.'])
  }
])
...
describe('bravura jira preset', function () {
  it('should work if there is no semver tag', function (done) {
    ...
    conventionalChangelogCore({
      ...
      .pipe(through(function (chunk) {
        ...
        expect(chunk).to.include(', closes [#1](https://github.com/ztan/conventional-changelog-jira/issues/1) [#2](https://github.com/ztan/conventional-changelog-jira/issues/2)')
  ...
  it('should remove the issues that already appear in the subject', function (done) {
    ...
    conventionalChangelogCore({
      ...
      .pipe(through(function (chunk) {
        chunk = chunk.toString()
        expect(chunk).to.include('[#88](https://github.com/ztan/conventional-changelog-jira/issues/88)')
        expect(chunk).to.not.include('closes [#88](https://github.com/ztan/conventional-changelog-jira/issues/88)')
        done()
      }))
  })
  ...
  it('should fallback to the closest package.json when not providing a location for a package.json', function (done) {
    ...
    conventionalChangelogCore({
      ...
      .pipe(through(function (chunk, enc, cb) {
        ...
        expect(chunk).to.include('(https://github.com/ztan/conventional-changelog-jira/compare')
        expect(chunk).to.include('](https://github.com/ztan/conventional-changelog-jira/commit/')
        expect(chunk).to.include('](https://github.com/ztan/conventional-changelog-jira/issues/')
